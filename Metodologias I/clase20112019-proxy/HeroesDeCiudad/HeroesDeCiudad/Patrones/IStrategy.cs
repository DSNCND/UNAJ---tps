...
namespace HeroesDeCiudad.Patrones
{
    ...
    class Espiral : IStrategy
    {
        ...
        /*
        void derecha(int[][]sectores,int caudal,int fOrigen, int cOrigen, int fDestino, int cDestino)
        {
            for (int i=cOrigen;i<=cDestino;i++)
            {
                Console.WriteLine("sector {0}{1} :{2}%",fOrigen,i,sectores[fOrigen][i]);
                while (sectores[fOrigen][i] > 0)
                {   
                    sectores[fOrigen][i] -= caudal;
                    if (sectores[fOrigen][i] < 0)
                    {
                        sectores[fOrigen][i] = 0;
                    }
        ...
        /*
        void abajo(int[][] sectores, int caudal, int fOrigen, int cOrigen, int fDestino, int cDestino)
        {
            for (int i = fOrigen; i <= fDestino; i++)
            {
                Console.WriteLine("sector {0}{1} :{2}%", i, cOrigen, sectores[i][cOrigen]);
                while (sectores[cOrigen][i] > 0)
                {
                    sectores[cOrigen][i] -= caudal;
                    if (sectores[i][cOrigen] < 0)
                    {
                        sectores[i][cOrigen] = 0;
                    }
        ...
        */
        /*
        void izquierda(int[][] sectores, int caudal, int fOrigen, int cOrigen, int fDestino, int cDestino)
        {
            for (int i = cOrigen; i >= cDestino; i--)
            {
                while (sectores[fOrigen][i] > 0)
        ...
        */
        /*
        void arriba(int[][] sectores, int caudal, int fOrigen, int cOrigen, int fDestino, int cDestino)
        {
            for (int i = cOrigen; i >= cDestino; i--)
            {
                while (sectores[fOrigen][i] > 0)
